...
namespace vMotion.Api.Specs.Unit_Tests._Features_.Agents.Calls.Post.PaymentIntent;
...
public class AutoMapperProfileTests : AutoFixtureTests<IMapper>
{
    ...
    {
        f.Customize<vMotion.api._Features_.Agents.Calls.Post.PaymentIntent.Request>(_ => _
           .With(x => x.CallId)
           .With(x => x.CardId)
           .With(x => x.Amount)
           .With(x => x.Currency, "cad") ////, DB.Entity<StaffEntity>().GenerateNewID())
           .With(x => x.DaysToHold, 14)
           .WithAutoProperties()
        );
    }
